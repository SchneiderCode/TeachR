...
$(document).ready(function () {
  ...
  //Read meta data from LearnR's Shiny Server
  Shiny.addCustomMessageHandler("set_id", setStudentID);

//TODO: Move monitor URL into the parameters list
  let monitorURL = $('meta[name=server-url]').attr('content')
  let tutorial_id = $('meta[name=tutorial-id]').attr('content')
  let titleText = ''
  ...
  function handleTopicClick (event) {
    ...
    fetch(`${monitorURL}?type=1&topic=${event.target.hash.substring(1, event.target.hash.length)}&section=`, {method: "GET"})
           .then(response => response.json()) // Parse JSON response
            .then(data => {
              // Check if the topic can be accessed
              if(data.can_proceed){
                updateLocation(this.getAttribute('index'));

              }else{//Unable to Proceed
                 bootbox.alert("Please wait for this topic to be released.");
              }
            })
            .catch(error => {
  ...
  function handleSkipClick (event) {
    ...
    if (section.exercises.length && !section.completed && !section.allowSkip) {
      ...
      fetch(`${monitorURL}?type=1&topic=${topic.id}&section=${section.id}`, {method: "GET"})
           .then(response => response.json()) // Parse JSON response
            .then(data => {
              ...
              if(data.can_proceed){
                ...
                tutorial.skipSection(sectionId)
                 ...
                 bootbox.alert("Please wait for next section to be released.");
              }
            })
            .catch(error => {
              console.error('Error fetching data:', error);
  ...
  function handleNextTopicClick (event) {
    ...
    fetch(`${monitorURL}?type=1&topic=${topic.id}&section=`, {method: "GET"})
           .then(response => response.json()) // Parse JSON response
            .then(data => {
              // Handle the parsed data here
              if(data.can_proceed){
  ...
  function buildTopicsList (topicAccess) {
    const topicsList = $(
    ...
    resetSectionVisibilityList()
    var topic_access_index = 0;
    const topicsDOM = $('.section.level2')
    topicsDOM.each(function (topicIndex, topicElement) {
      if($(topicElement).attr('id') in topicAccess){//Ensure the topic's id is listed
        if(topicAccess[$(topicElement).attr('id')].length !== 0){//And that the topic contains at Least 1 accessible section
          ...
          const jqTopic = $(
            `<li class="topic${isBS3() ? '' : ' nav-item'}" index="${topic_access_index}">` +
          ...
          if (topic_access_index < topicAccess.length - 1) {
            const nextButton = $(
        ...
        const sectionsDOM = $(topicElement).children('.section.level3')
        sectionsDOM.each(function (sectionIndex, sectionElement) {
          if(topicAccess[topic.id].includes(sectionElement.id)){
          ...
          else{//Hide inaccessible sections
            $(sectionElement).trigger('hide')
            $(sectionElement).addClass('hide')
            $(sectionElement).trigger('hidden')
          }
        })

        topics.push(topic)
        topic_access_index++;//Increment to prepare for the next accessible topic entry
        }
      }
    })
  ...
  function transformDOM (topicAccess) {
    ...
    $('.bandContent.topicsListContainer').append(buildTopicsList(topicAccess))
  ...
  fetch(`${monitorURL}?type=2`, {method: "GET"})
   .then(response => response.json()) // Parse JSON response
    .then(data => {
      // Store the list of topics & their accessible sections
      preTransformDOMMigrateFromBS3()
      transformDOM(data.access)
      handleLocationHash()
    })
    .catch(error => {
      // Handle any errors here
      console.error('Error fetching data:', error);
      alert("Error in accessing Tutorial Server!");
    });
